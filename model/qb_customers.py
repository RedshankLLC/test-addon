            ...
            if data.status_code == 429:
                ...
                if backend.company_id:
                    data = headeroauth.get(
                        backend.location + backend.company_id + '/' + arguments + '/' + str(record_id) + '?minorversion=4',headers=headers)
                else:
                    raise UserError(_('Please add company ID'))
    ...
    def customer_import_mapper(self, backend_id, data):
        ...
        if 'Customer' in record:
            ...
            if 'BillAddr' in rec:
                bil = rec['BillAddr']
                if bil:
                    street = bil['Line1']  if 'Line1' in bil else None
                    street2 = bil.get('Line2') if 'Line2' in bil else None
